...
原因：浏览器会把 `router.js?v=11` 和 `router.js?v=12` 视为两个不同模块，分别创建两份 `state`。导航可能更新其中一份状态，而页面由另一份状态渲染，表现为点击后又回到旧页面。

发布 JavaScript 或 CSS 修改时：

1. 将当前版本统一提升，例如 `v=11` 改为 `v=12`。
2. 使用 `rg "\\?v=11" index.html js` 找出全部引用。
3. 确认旧版本号没有残留。
